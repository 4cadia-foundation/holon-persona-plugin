...
import PersonaStorage from '../../../scripts/core/PersonaStorage';

const transactor = new Transactor();
var personaStorage = new PersonaStorage();
    ...
    try {
        ...
        if (!transactor.wallet) {            
            if (store.getState().wallet.ethersWallet) {
                ...
                console.log('persona/checkWallet/pk:', pk);
        ...
        console.error('persona/checkWallet/error: ', err);
        return false;
    }
...
export function checkPersonaStorage(dispatch) {
    console.log('action/persona/checkPersonaStorage/starting')
    personaStorage.getChromeStorage()
    .then ( (objPersonaData) => {
        console.log('action/persona/checkPersonaStorage/objPersonaData', objPersonaData);
        dispatch(objPersonaData);
        dispatch({ type: 'HAS_CHECKED_PERSONA_STORAGE' });
    })
    .catch ( (err) => {
        dispatch({ type: 'HAS_CHECKED_PERSONA_STORAGE' });
        console.log('actions/checkPersonaStorage/failed', err.message);    
    });
}
